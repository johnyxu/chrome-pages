...
2. 管理页面会提示"还没有连接任何文件"，点击**"连接一个文件"**。
   ...
   ![连接文件](images/manager-connect.svg)
...
![设置面板](images/popup-settings.svg)
...
点击弹出面板里的"查看已保存的链接"，打开管理页面。管理页面分为左侧的**收藏侧边栏**和右侧的**主内容区**。

### 列表视图

默认是列表视图，链接按**域名自动分组**显示：

![列表视图](images/manager-list.svg)
...
点击顶部的**"▦ Cards"**按钮，可以切换成卡片网格布局，同样按域名分组，只是每条链接以卡片形式并排展示，适合喜欢更直观、图块化浏览方式的场景：

![卡片视图](images/manager-card.svg)
...
![最少查看视图](images/manager-least-viewed.svg)
